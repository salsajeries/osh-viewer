...
import { when, Cartographic, Cartesian3, Cartesian2, Color, HorizontalOrigin, VerticalOrigin, HeightReference, Math, Transforms, Matrix3, Matrix4, MaterialAppearance, Material, sampleTerrain, GeometryInstance, PolygonGeometry, RectangleGeometry, Rectangle, Primitive, createDefaultImageryProviderViewModels, WebMapServiceImageryProvider, EllipsoidTerrainProvider, NearFarScalar, HeadingPitchRoll, HeadingPitchRange, Ellipsoid, EncodedCartesian3, ScreenSpaceEventType, ColorGeometryInstanceAttribute, Scene, PolygonHierarchy, PerInstanceColorAppearance, GroundPrimitive, PolylineCollection, PrimitiveCollection, EllipseGeometry, BillboardCollection, LabelCollection, Entity, Quaternion, ColorBlendMode, PerspectiveFrustum, FrustumGeometry, VertexFormat, CoplanarPolygonGeometry, GroundPolylineGeometry, GroundPolylinePrimitive, PolylineGeometry, PolylineColorAppearance, LabelStyle, EllipsoidGeometry } from 'cesium';
...
var CesiumView = /** @class */ (function (_super) {
    ...
    CesiumView.prototype.beforeAddingItems = function (options) {
        ...
        if (options && options.options && options.options.layers) {
            var imageryFilter_1 = options.options.layers;
            imageryProviders = imageryProviders.filter(function (el) { return imageryFilter_1.includes(el.name); });
        }
        var viewerProps = {
    ...
    CesiumView.prototype.initCallbackEvents = function () {
        ...
        var onClick = function (movement) {
            try {
                ...
                var layerObject = that.getLayerObject(layerId);
                if (isDefined(entity)) {
    ...
    CesiumView.prototype.removeMarkerFromLayer = function (entity, markerId) {
...
//# sourceMappingURL=CesiumViewNew.js.map