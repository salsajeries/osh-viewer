...
import DataSourceWorker from './worker/DataSource.worker.js?worker'
import WorkerExt from '../worker/WorkerExt.js'
...
class DataSource {
    ...
    createWorker(properties) {
       /* const workerUrl = new URL('./worker/DataSource.worker.js?worker', import.meta.url);
        console.log('[DataSource] Creating worker for DataSource: ' + this.id + ' at ' + workerUrl);
        const worker = new Worker(workerUrl, {type: 'module'});
        console.log('[DataSource] Worker created for DataSource: ', worker);
        const workerExt = new WorkerExt(worker);*/

        const workerExt = new WorkerExt(new DataSourceWorker());
        console.log('[DataSource] WorkerExt created for DataSource: ', workerExt);
        return workerExt;
    }
